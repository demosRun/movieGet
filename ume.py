...
def umeGetOrder(body):
    url = "https://oc.yuekeyun.com/api/storeServer/storeTkOrderHeaderService/findStoreTkOrderInfoApp"
    payload = 'params={"orderType":"ticket_order","isDetail":"Y","orderHeaderId":"%s","channelCode":"QD0000001"}' % (body["orderID"])
    headers = {
    'Host': 'oc.yuekeyun.com',
    'content-type': 'application/x-www-form-urlencoded',
    'tenantCode': 'cinema_umedy',
...
def umeSeatMapInfo(CinemaData, body):
    FilmData = findFilmInfoToApp(CinemaData, body["filmName"])
    if (FilmData != None):
        ScheduleData = findScheduleInfoToApp(CinemaData, FilmData, body["showDate"], body["showTime"])
        if (ScheduleData != None):
